...
namespace Training.Specificaton
{
    public static class AssertionExtensions
    {
        ...
        public static void ShouldContainOnlyInOrder<TItem>(this IEnumerable<TItem> items,
            IEnumerable<TItem> ordered_items)
        {
            List<TItem> source = new List<TItem>(items);
            if (!Enumerable.Any<TItem>(Enumerable.Where<TItem>(ordered_items,
                    (Func<TItem, int, bool>) ((ordered_element, index) =>
                        !source[index].Equals((object) ordered_element)))))
                return;
            throw new SpecificationException(string.Format(
                ...
                ordered_items.EachToUsefulString(), items.EachToUsefulString()));
        }
    }
}